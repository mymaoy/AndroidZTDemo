...
public final class CameraManager {
     ...
     */
    public boolean openDriver() throws IOException {//SurfaceHolder holder
        if (mCamera == null) {
            try {

                Log.v("ABC","IN Camera.open(0)");
                mCamera = Camera.open(0);
                if (mCamera != null) {
                    // setParameters 是针对魅族MX5做的。MX5通过Camera.open()拿到的Camera 对象不为null
                    Camera.Parameters mParameters = mCamera.getParameters();
                    Log.v("ABC","IN Camera.open(0)  OK");
...
//                        Log.i("PASSA","pictureSize="+psize.width+" x "+psize.height);
//                    }
//                    for (int i = 0; i < previewSizes.size(); i++) {
//                        psize = previewSizes.get(i);
//                        Log.i("PASSA","previewSize="+psize.width+" x "+psize.height);
//                    }

                  //  mParameters.setPictureSize(1280,720);
                   // mParameters.setPreviewSize(1280,720);
                    Log.v("ABC","w="+mParameters.getPictureSize().width+",h="+mParameters.getPictureSize().height);
...
//                    String currentFocusMode = mCamera.getParameters().getFocusMode();
                   ...
                   mConfigManager.setDesiredCameraParameters(mCamera);
                    ...
                    Log.v("ABC","IN Camera.open(0) ERROR");
    ...
    public boolean startPreview() {
        ...
        Log.v("ABC","startPreview --1");
        if (mCamera != null && !mPreviewing) {
            try {
                Log.v("ABC","startPreview --2");
